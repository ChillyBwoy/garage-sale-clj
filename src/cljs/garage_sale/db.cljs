(ns garage-sale.db)

(def default-db
  {
   :games [{:id "BLES00926"
            :sold? false
            :name "Batman: Arkham City"
            :comment ""
            :price 300.0}
           {:id "BLES01211"
            :sold? false
            :name "Binary Domain"
            :comment ""
            :price 300.0}
           {:id "BLES01047"
            :sold? false
            :name "Castlevania: Lords of Shadow"
            :comment ""
            :price 300.0}
           {:id "BLES00311"
            :sold? false
            :name "Chronicles of Riddick: Assault on Dark Athena"
            :comment ""
            :price 300.0}
           {:id "BLES01388"
            :sold? false
            :name "Darkness II"
            :comment ""
            :price 300.0}
           {:id "BLES01597"
            :sold? false
            :name "Darksiders II"
            :comment ""
            :price 300.0}
           {:id "BCAS20115"
            :sold? false
            :name "Demon’s Souls"
            :comment ""
            :price 300.0}
           {:id "BLES01150"
            :sold? false
            :name "Deus Ex: Human Revolution"
            :comment ""
            :price 300.0}
           {:id "BLES01356"
            :sold? false
            :name "Dragon’s Dogma"
            :comment ""
            :price 300.0}
           {:id "BLES01329"
            :sold? false
            :name "Elder Scrolls V: Skyrim"
            :comment ""
            :price 400.0}
           {:id "BLES00989"
            :sold? false
            :name "Enslaved: Odyssey to the West"
            :comment "OST+ArtBook"
            :price 800.0}
           {:id "BLES00783"
            :sold? false
            :name "Final Fantasy XIII"
            :comment ""
            :price 300.0}
           {:id "BCES00050"
            :sold? false
            :name "Folklore"
            :comment ""
            :price 300.0}
           {:id "BLES00788"
            :sold? false
            :name "Front Mission Evolved"
            :comment ""
            :price 300.0}
           {:id "BCES00510"
            :sold? false
            :name "God of War III"
            :comment "Collector’s Edition"
            :price 500.0}
           {:id "BLUS30127"
            :sold? false
            :name "Grand Theft Auto IV"
            :comment ""
            :price 300.0}
           {:id "BLES01807"
            :sold? false
            :name "Grand Theft Auto V"
            :comment ""
            :price 400.0}
           {:id "BCES00078"
            :sold? false
            :name "Heavenly Sword"
            :comment ""
            :price 400.0}
           {:id "BCES00802/P/RUS"
            :sold? false
            :name "Heavy Rain: Move Edition"
            :comment ""
            :price 400.0}
           {:id "BCES01143"
            :sold? false
            :name "inFamous 2"
            :comment ""
            :price 300.0}
           {:id "BLES00517"
            :sold? false
            :name "Just Cause 2"
            :comment ""
            :price 400.0}
           {:id "BLES00658"
            :sold? false
            :name "Katamari forever"
            :comment ""
            :price 300.0}
           {:id "BCES01007"
            :sold? false
            :name "Killzone 3"
            :comment ""
            :price 400.0}
           {:id "BLES00933"
            :sold? false
            :name "L.A. Noire"
            :comment ""
            :price 300.0}
           {:id "BCES0004"
            :sold? false
            :name "Lair"
            :comment ""
            :price 400.0}
           {:id "BCES01585"
            :sold? false
            :name "Last of Us"
            :comment ""
            :price 400.0}
           {:id "BLES00198"
            :sold? false
            :name "Lost Planet: Extreme Condition"
            :comment ""
            :price 300.0}
           {:id "BLES00940"
            :sold? false
            :name "Mafia II"
            :comment ""
            :price 400.0}
           {:id "BLUS30650"
            :sold? false
            :name "Mass Effect 2"
            :comment ""
            :price 300.0}
           {:id "BCES01279"
            :sold? false
            :name "Medieval Moves: Боевые Кости"
            :comment ""
            :price 300.0}
           {:id "BCES00006"
            :sold? false
            :name "MotorStorm"
            :comment ""
            :price 300.0}
           {:id "BCES00129"
            :sold? false
            :name "MotorStorm: Pacific Rift"
            :comment ""
            :price 300.0}
           {:id "BLES00736"
            :sold? false
            :name "Ninja Gaiden Sigma 2"
            :comment ""
            :price 400.0}
           {:id "BCED01063"
            :sold? false
            :name "PS Move: Starter Disc"
            :comment ""
            :price 300.0}
           {:id "BLES01377"
            :sold? false
            :name "Rage: Anarchy Edition"
            :comment ""
            :price 400.0}
           {:id "BCES00511/P"
            :sold? false
            :name "Ratchet & Clank Future: A Crack in Time"
            :comment ""
            :price 300.0}
           {:id "BLES01386"
            :sold? false
            :name "Rayman Origins"
            :comment ""
            :price 300.0}
           {:id "BLES00680"
            :sold? false
            :name "Red Dead Redemption"
            :comment ""
            :price 400.0}
           {:id "BCES0001"
            :sold? false
            :name "Resistance: Fall of Man"
            :comment "Самая первая игра для PS3"
            :price 300.0}
           {:id "BLUS30670"
            :sold? true
            :name "Rocksmith"
            :comment ""
            :price 500.0}
           {:id "BLES01371"
            :sold? false
            :name "Syndicate"
            :comment ""
            :price 300.0}
           {:id "BCES00757/P"
            :sold? true
            :name "Uncharted 2: Among Thieves"
            :comment ""
            :price 400.0}
           {:id "BCES01176"
            :sold? true
            :name "Uncharted 3: Drake's Deception"
            :comment ""
            :price 400.0}
           {:id "BLES00927"
            :sold? false
            :name "Vanquish"
            :comment ""
            :price 400.0}
           {:id "BLES01712"
            :sold? false
            :name "XCOM: Enemy Unknown"
            :comment ""
            :price 300.0}]})
